export type ReplyResult = [text: string, goodbye: boolean];

// --- helpers (ported from Go) ---
// preprocess: trim, lower-case, strip trailing punctuation (.,!?'")
...
// random chooser
function randomElementFrom<T>(list: T[]): T {
  return list[Math.floor(Math.random() * list.length)];
}

// reflect pronouns in a captured fragment
function reflect(fragment: string): string {
  const words = fragment.split(/\s+/);
  for (let i = 0; i < words.length; i++) {
    const w = words[i];
    const r = reflectedWords[w];
    if (r) words[i] = r;
  }
  return words.join(" ");
}

// main lookup (regex → response options)
function lookupResponse(input: string): string {
  for (const [re, responses] of requestInputRegexToResponseOptions) {
    const m = input.match(re);
    if (!m) continue;

    let response = randomElementFrom(responses);
    if (!response.includes("%s")) {
      return response;
    }
    if (m[1]) {
      const fragment = reflect(m[1]);
      return response.replace("%s", fragment);
    }
  }
  return randomElementFrom(defaultResponses);
}

// exported: reply(input) → [text, goodbye?]
export function reply(input: string): ReplyResult {
  const norm = preprocess(input); // like Go preprocess() :contentReference[oaicite:2]{index=2}
  if (goodbyeInputSet.has(norm)) {
    return [randomElementFrom(goodbyeResponses), true]; // goodbye branch from Go Reply() :contentReference[oaicite:3]{index=3}
  }
  return [lookupResponse(norm), false];
}

// exported: intro lines (like GetIntroResponses(name)) :contentReference[oaicite:4]{index=4}
export function getIntroResponses(name: string): string[] {
  const intros = introResponses.map((tpl) => tpl.replace("%s", name));
  intros.push(randomElementFrom(facts));
  intros.push("How are you feeling today?");
  return intros;
}

// ---- data ported from globals.go ---- :contentReference[oaicite:5]{index=5}

const goodbyeInputSet = new Set<string>(["bye", "exit", "goodbye", "quit"]);
...
const requestInputRegexToResponseOptions: Array<[RegExp, string[]]> = [
  [
    /^i need (.*)$/,
    [
      "Why do you need %s?",
  ...
  ], // note: lowercased to match preprocess()
  [
    /(.*) friend (.*)/,
    [
...
const reflectedWords: Record<string, string> = {
  am: "are",
  was: "were",
  i: "you",
  "i'd": "you would",
